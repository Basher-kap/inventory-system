          ...
          <div className={`relative w-full max-w-md p-6 sm:p-10 backdrop-blur-3xl border rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl text-center ${isDarkMode ? 'bg-gradient-to-br from-white/10 to-[#050B14] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <button
                ...
                className={`flex-1 py-3 sm:py-4 rounded-xl sm:rounded-2xl border transition-all font-bold cursor-pointer ${isDarkMode ? 'border-white/10 hover:bg-white/5 text-white' : 'border-slate-200 hover:bg-slate-100 text-slate-700'}`}
              >
                Cancel
              </button>
              <button
                onClick={confirmReturn}
                className="flex-1 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-[#3852A4] text-white font-bold hover:bg-[#2e438a] transition-all cursor-pointer shadow-lg active:scale-95"
              >
                Confirm
              </button>
            </div>
                    ...
                    <td className="px-6 sm:px-8 py-6 sm:py-8 text-right whitespace-nowrap">
                      {log.status === 'Active' ? (
                        <button
                          ...
                          className={`px-4 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl font-bold text-[10px] sm:text-sm tracking-widest uppercase transition-all shadow-sm active:scale-95 cursor-pointer flex items-center justify-end gap-2 ml-auto border
                            ${isDarkMode
                        ...
                        >
                          <svg width="14" height="14" sm:width="16" sm:height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                          Receive Return
                        </button>
                      ) : (
